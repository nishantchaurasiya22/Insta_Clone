...
from app.services.follow_service import send_follow_request_service,accept_follow_request_service,reject_follow_request_service
...
@follow_router.patch("/accept/{following_id}",response_model=ResponseFollow,status_code=status.HTTP_200_OK)
def accept_request(following_id:int,current_user:dict=Depends(get_current_user)):
    follower_id=int(current_user["sub"])
    try:
...
@follow_router.delete("/reject/{following_id}",response_model=ResponseFollow,status_code=status.HTTP_200_OK)
def reject_request(following_id:int,current_user:dict=Depends(get_current_user)):
    follower_id=int(current_user["sub"])
    try:
